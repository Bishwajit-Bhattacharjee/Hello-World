...
double angle(PT a, PT b, PT c )
{
    ...
    double ab = dist(a,b) ;
    double bc = dist(b,c) ;
    double tmp = (ab*ab + bc * bc - ac*ac)/ (2 * ab * bc ) ;
    return acos(tmp) * 180 / acos(-1.0) ;
}
...
int main()
{
    ...
    while(t--)
    {
     ...
     if(h.size() < 3  ) {
        ...
        ans = min (ans ,angle(h[(i-1+h.size())% h.size()], h[i] ,h[(i+1) % h.size()] ) ) ;
      }
    // ans += len ;
    // cout << "Case " << ++c << ": "<< setprecision(10) << fixed << ans << '\n' ;
  //  printf("Case %d: %.10lf\n",++c, ans ) ;
